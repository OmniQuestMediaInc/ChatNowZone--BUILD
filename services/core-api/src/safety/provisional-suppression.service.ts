...
// ---------------------------------------------------------------------------
// STUB: Provisional suppression store.
// Tracks PROVISIONAL suppressions pending human moderator review.
// WARNING: This in-memory map does NOT persist across service restarts.
// Production deployments MUST replace this with a database-backed table
// (e.g., `content_suppression_queue`) before go-live.
// ---------------------------------------------------------------------------
const provisionalSuppressions = new Map<string, SuppressionRecord>();
...
export class ProvisionalSuppressionService {
  ...
  async suppressContent(
    ...
    provisionalSuppressions.set(contentId, record);
  ...
  async liftSuppression(contentId: string, moderatorId: string): Promise<void> {
    const record = provisionalSuppressions.get(contentId);
  ...
  async finalizeAndRegisterHash(
    ...
    if (!record || record.status !== 'PROVISIONAL') {
      throw new Error(
  ...
  async generateEvidencePacket(caseId: string): Promise<EvidencePacket> {
    ...
    // Include provisional suppression events for this case in the timeline
    provisionalSuppressions.forEach((record) => {
